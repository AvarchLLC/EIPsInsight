...
const CBoxStatus:React.FC<CBoxProps> = ({status}) => {
    ...
    const statusData = data.filter(item => item.status === getStatus(status));
    const yearData = statusData.flatMap(item => item.eips).filter(item => item.year === selectedYear );

    const result: { [key: string]: number } = {};

    yearData.forEach(item => {
        item.eips.forEach(item => {
            if ((item.type === "Standards Track" || item.type === "Standard Track" || item.type == "Standard" || item.type == "Core" || item.type == "Process" || item.type == "Standards Track (Core, Networking, Interface, ERC)") && item.status===getStatus(status)) {
            ...
            } else if (item.type === "Informational" && item.status===getStatus(status)) {
    ...
    const convertAndDownloadCSV = () => {
        if (yearData && yearData.length > 0) {
            ...
            const columnsToDrop = ['_id', 'fromStatus', 'toStatus', 'changeDate', 'type','discussion','requires','changedDay','changedMonth','changedYear','createdMonth','deadline', 'createdYear','__v'];

            // Create CSV headers
            const headers = Object.keys(yearData[0].eips[0])
            ...
            const csvRows = yearData.map((item) => {
                const values = item.eips.map((eip) => {
                    ...
                    const filteredValues = Object.values(eip)
                        .filter((value, index) => !columnsToDrop.includes(Object.keys(eip)[index]))
                        .map((value) => {
    ...
    const years = Array.from(new Array(currentYear - 2014), (val, index) => index + 2015).reverse();
    ...
    return (
        ...
        >
            ...
            <Box className={'flex w-full gap-10'}>
                ...
                <Box>
                    <Button colorScheme="blue" variant="outline" fontSize={'14px'} fontWeight={'bold'} padding={'8px 20px'} onClick={convertAndDownloadCSV}>
            ...
            <Box className={'flex justify-center w-full text-center'}>
                <Text fontSize="xl" fontWeight="bold" color="#30A0E0" marginRight="6">Total: {total}</Text>
            </Box>
