...
const EIPCatBoxGrid = () => {
    ...
    const InformationalCount = data.filter((item) => item.type === "Informational").length;
    ...
    return(
        <>
            <Box display={'grid'} gridTemplateColumns={'repeat(4,1fr)'} gap={8} bgColor={bg} className={'p-4 rounded-lg mt-8'}>
                <CatBox
                    title="Meta"
                    value={MetaCount}
            ...
            <Text fontSize="3xl" fontWeight="bold" color="#30A0E0" paddingY={8}>
                Standards Track -{" "}
                {data.filter((item) => item.type === "Standards Track"  && item.category !== 'ERC' ).length}
            </Text>

            <Box display={'grid'} gridTemplateColumns={{md:'repeat(6,1fr)',base:'repeat(4,1fr)'}} gap={8} bgColor={bg} className={'p-4 rounded-lg'}>
                <CatBox
                    title="Core"
                    value={CoreCount}
                    icon={<Icon as={Link} />}
