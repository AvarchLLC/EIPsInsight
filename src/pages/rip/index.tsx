...
import { Box, Button, Grid, Text, useColorModeValue,IconButton, Flex , Collapse, Heading, useDisclosure, Link as LI, ButtonGroup} from "@chakra-ui/react";
          ...
          <Box key={status} className={"group relative flex flex-col gap-3"} paddingBottom={8}>
            ...
            <Box overflowX="auto">
              <Grid templateColumns={{ base: "1fr", sm: "1fr", lg: "repeat(2, 1fr)" }} gap={6}>
                <StackedColumnChart type={"RIPs"} status={status} dataset={data2} />
                <CBoxStatus status={status} type={"RIPs"} dataset={data3} />
              </Grid>
            </Box>
          </Box>
        ))}
